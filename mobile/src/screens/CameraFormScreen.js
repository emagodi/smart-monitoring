...
const CameraFormScreen = ({ route, navigation }) => {
    const { transformerId, camera } = route.params || {};
    ...
    useEffect(() => {
        if (isEditing && camera) {
            ...
            navigation.setOptions({ title: 'Edit Camera' });
        } else {
            navigation.setOptions({ title: 'Add Camera' });
        }
    }, [camera, isEditing, navigation]);

    const handleSubmit = async () => {
        // Validation
        if (!formData.name) {
            Alert.alert('Validation Error', 'Camera Name is required');
            return;
        }
        if (!formData.topic) {
            Alert.alert('Validation Error', 'MQTT Topic is required');
            return;
        }
        ...
        try {
            ...
            if (isEditing) {
                ...
                await cameraService.registerCamera(payload);
                ...
                <View style={styles.formCard}>
                    <View style={styles.header}>
                        <View style={styles.iconContainer}>
                            <Ionicons name="camera" size={32} color="#0067A5" />
                        </View>
                    </View>
                    ...
                    <View style={styles.inputGroup}>
                        ...
                        <TextInput
                            ...
                            placeholder="e.g. transformer/123/cam1"
                            placeholderTextColor="#999"
                            autoCapitalize="none"
                        />
                        <Text style={styles.helperText}>Used for real-time communication</Text>
                    </View>
...
export default CameraFormScreen;
